...
/**
 * Seed default pets data
 */
const seedDefaultPets = asyncHandler(async (req, res) => {
  // Get shelter ID from authenticated user
  const shelterId = req.user.userId;
  
  console.log('Seeding default pets for shelter:', shelterId);
  ...
  successResponse(res, {
    message: `Successfully added ${successCount} pets${errorCount > 0 ? `, ${errorCount} failed` : ''}`,
    addedPets,
    successCount,
    errorCount,
  }, 'Default pets seeded successfully', 201);
});
